<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dv="http://namespaces.plone.org/xdv" xmlns:esi="http://www.edge-delivery.org/esi/1.0" xmlns:exsl="http://exslt.org/common" xmlns:xhtml="http://www.w3.org/1999/xhtml" version="1.0" exclude-result-prefixes="exsl dv xhtml esi">
  <xsl:output method="xml" indent="no" omit-xml-declaration="yes" media-type="text/html" encoding="utf-8" doctype-public="-//W3C//DTD XHTML 1.0 Transitional//EN" doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"/>
    ...
    <xsl:template match="/" mode="apply-theme">
        ...
        <xsl:copy-of select="document('extra.html', .)//*[@id='external1']"/><xsl:copy-of select="document('extra.html', .)//*[@id='external2']"/><xsl:copy-of select="document('extra.html', .)//*[@id='external3']"/>
        <div id="marker2"><xsl:copy-of select="document('extra.html', .)//*[@id='external4']"/><xsl:copy-of select="document('extra.html', .)//*[@id='external5']"/><xsl:copy-of select="document('extra.html', .)//*[@id='external6']"/></div><xsl:copy-of select="/html/body/*"/>
    </body></html>
    </xsl:template>
